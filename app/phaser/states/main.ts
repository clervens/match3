...
class Main extends Phaser.State {
	create() {
		...
		me.tileWidth = me.game.cache.getImage('black').width;
		me.tileHeight = me.game.cache.getImage('black').height;
		...
		me.tileWidth = Math.max(this.game.width/me.tileGrid[0].length, me.tileWidth);
		me.tileHeight = Math.max(this.game.height/(me.tileGrid.length + 1), me.tileHeight);
	...
	private addTile(x, y):any {
		...
		me.game.add.tween(tile).to({y:y*me.tileHeight+(me.tileHeight/2)}, 500, Phaser.Easing.Linear.None, true)
	...
	private swapTiles(){
		...
		if(me.activeTile1 && me.activeTile2){
			...
			let tile1Pos = {x:Math.floor((me.activeTile1.x - me.tileWidth / 2) / me.tileWidth), y:Math.floor((me.activeTile1.y - me.tileHeight / 2) / me.tileHeight)};
			let tile2Pos = {x:Math.floor((me.activeTile2.x - me.tileWidth / 2) / me.tileWidth), y:Math.floor((me.activeTile2.y - me.tileHeight / 2) / me.tileHeight)};
	...
	private getMatches(tileGrid): any[] {
		let matches = [];
		let groups = [];
		...
		for (let i = 0; i < me.tileGrid.length; i++)
		{
			...
			for (let j = me.tileGrid[i].length - 1; j > 0; j--)
			{
				...
				if(me.tileGrid[i][j] == null && me.tileGrid[i][j-1] != null)
				{
					...
					me.game.add.tween(tempTile).to({y:(me.tileHeight*j)+(me.tileHeight/2)}, 200, Phaser.Easing.Linear.None, true);
